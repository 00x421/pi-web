...
const { findExistingAttachment, isSafeAttachmentName, uniqueAttachmentName } = await jiti.import("./attachments.ts");
...
test("reuses an existing copy of the same file instead of stacking duplicates", () => {
  const existing = [{ name: "report.pdf", size: 2048 }];
  assert.equal(findExistingAttachment({ name: "report.pdf", size: 2048 }, existing), "report.pdf");
  assert.equal(findExistingAttachment({ name: "report.pdf", size: 999 }, existing), null);
  assert.equal(findExistingAttachment({ name: "other.pdf", size: 2048 }, existing), null);
  assert.equal(findExistingAttachment({ name: "report.pdf", size: 2048 }, []), null);
});
