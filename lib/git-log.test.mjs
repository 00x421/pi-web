...
test("clamps the requested limit", () => {
  ...
  assert.equal(clampGitLogLimit("50"), 50);
  assert.equal(clampGitLogLimit(12.7), 12);
});

// A missing ?limit= arrives as null, and Number(null) is 0 — which used to clamp
// the history down to a single commit.
test("treats a missing or unusable limit as the default", () => {
  for (const missing of [null, undefined, "", "   ", "abc", Number.NaN, {}]) {
    assert.equal(clampGitLogLimit(missing), GIT_LOG_DEFAULT_LIMIT, `limit: ${String(missing)}`);
  }
});
